  ...
  return (
    <div className="space-y-6">
      <header className="flex items-end justify-between flex-wrap gap-4">
        <div>
          <h1 className="font-display text-3xl font-semibold">Team calendar</h1>
          <p className="text-muted-foreground text-sm mt-1">Who's doing what this week.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="icon" onClick={() => setWeekStart(addDays(weekStart, -7))}><ChevronLeft className="h-4 w-4" /></Button>
          <span className="text-sm font-medium px-3">{weekStart.toLocaleDateString("en-GB", { day: "numeric", month: "short" })} – {addDays(weekStart, 6).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</span>
      ...
      <div className="flex flex-wrap gap-3">
        <Select value={filterType} onValueChange={setFilterType}>
          <SelectTrigger className="w-48"><SelectValue placeholder="All task types" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={filterUser} onValueChange={setFilterUser}>
          <SelectTrigger className="w-56"><SelectValue placeholder="All volunteers" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All volunteers</SelectItem>
            {volunteers.map((v) => <SelectItem key={v.id} value={v.id}>{v.full_name || "Unnamed"}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
