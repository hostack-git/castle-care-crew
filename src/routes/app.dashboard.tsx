...
function VolunteerDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Find this volunteer's record — first by auth_user_id, then by name
      let volunteerIdFilter: string | null = null;
      ...
      if (volByAuth?.id) {
        volunteerIdFilter = volByAuth.id;
      } else {
        ...
        if (fullName) {
          const { data: volByName } = await hostackSupabase
            ...
            .ilike("name", fullName)
            .maybeSingle();
          if (volByName?.id) volunteerIdFilter = volByName.id;
        }
      }
      ...
      if (volunteerIdFilter) {
        query = query.eq("volunteer_id", volunteerIdFilter);
      }
