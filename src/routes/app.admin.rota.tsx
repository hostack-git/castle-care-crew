...
import { ChevronLeft, ChevronRight, Save, ArrowLeft, CalendarDays } from "lucide-react";
import { toast } from "sonner";

export const Route = createFileRoute("/app/admin/rota")({ component: RotaBuilderPage });

type Volunteer = { id: string; name: string | null };
...
// Color por tipo (matched por nombre, lowercase contains)
const TYPE_COLORS: { match: string; cls: string }[] = [
  ...
  { match: "maintenance", cls: "bg-yellow-100 text-yellow-900 border-yellow-200" },
...
const EMPTY_CLS = "bg-background text-muted-foreground/60 border-dashed border-border";
...
function RotaBuilderPage() {
  ...
  // grid[volunteer_id][date_ymd] = shift_template_id | OFF_KEY | null
  ...
  const [shiftIds, setShiftIds] = useState<Record<string, string>>({}); // key `${vid}_${date}` -> shift.id
  ...
  useEffect(() => {
    ...
    setLoadingData(true);
    (async () => {
      const [volRes, tplRes, shiftRes, bookRes] = await Promise.all([
        hostackSupabase
          .from("volunteers")
          .select("id, name")
      ...
      const bookings = (bookRes.data as { check_in: string }[]) ?? [];
      ...
      for (const b of bookings) counts[b.check_in] = (counts[b.check_in] ?? 0) + 1;
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-muted/40">
                <th className="text-left px-3 py-2 sticky left-0 bg-muted/40 z-10 min-w-[180px] border-b">
                  Voluntario
                </th>
                {days.map((d) => {
                  const dateStr = ymd(d);
                  ...
                  return (
                    <th key={dateStr} className="px-2 py-2 text-center font-medium border-b min-w-[140px]">
                      <div className="flex flex-col items-center">
                        <span>{DAY_LABELS_ES[d.getDay() === 0 ? 6 : d.getDay() - 1]} {d.getDate()}</span>
            ...
            <tbody>
              {volunteers.map((v) => (
                <tr key={v.id} className="border-b last:border-0">
                  <td className="px-3 py-2 sticky left-0 bg-card z-10 font-medium border-r">
                    {v.name ?? "—"}
                  </td>
                  {days.map((d) => {
                    const date = ymd(d);
                    ...
                    const label = value === OFF_KEY ? "Off" : tpl?.name ?? "–";
                    const cls = value === OFF_KEY ? OFF_CLS : tpl ? colorFor(tpl.name) : EMPTY_CLS;
                    return (
